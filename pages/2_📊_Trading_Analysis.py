...
if st.session_state.get("dark_mode", True):
    ...
    <style>
        ...
        @media (max-width: 768px) { .stApp { padding: 0.5rem; } }
        .stSelectbox > div > div { 
            background-color: rgba(30, 35, 41, 0.8);
            color: #fafafa;
        }
        .stSlider > div > div { color: #fafafa !important; }
    ...
    <style>
        ...
        .stSlider > div > div { color: #262730 !important; }
        [data-testid="metric-container"] {
            background-color: rgba(255, 255, 255, 0.9);
...
with st.sidebar:
    ...
    if st.button(f"{theme_icon} {theme_text}", key="theme_toggle_trading"):
            ...
            if "error" in analysis:
                ...
                try:
                    ...
                    if chart_df is None or chart_df.empty:
                        st.warning("No data available for charting. Please check the symbol and try again.")
                    else:
                        ...
                        if not chart_df.empty:
                            ...
                            # Add error handling for chart display
                            try:
                                st.plotly_chart(fig, use_container_width=True)
                            except Exception as chart_error:
                                st.error(f"Error displaying chart: {str(chart_error)}")
                                st.info("Chart data is available but there was an issue displaying it. Please try refreshing the page.")
                        else:
